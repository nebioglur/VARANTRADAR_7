...
def notify_sim_trade(symbol: str, action: str, price: float, pnl_pct: float = 0.0, reason: str = "", date_str: str = "", trade: dict = None) -> bool:
    ...
    if "AL" in action:
        text = "SIMULASYON " + action + "{nl}{nl}"
        text += symbol + " -> " + f"{{price:.2f}}" + " TL{nl}"
        text += "Lot Sayisi: " + str(shares) + " Lot{nl}"
        text += "Toplam Tutar: " + f"{{total_val:.2f}}" + " TL{nl}{nl}"
        text += "Kar Al (TP): " + f"{{tp1:.2f}}" + " TL (+%" + f"{{tp_pct:.1f}}" + "){nl}"
        text += "Stop Sat (SL): " + f"{{sl:.2f}}" + " TL (-%" + f"{{sl_pct:.1f}}" + "){nl}"
        if reason:
            text += "{nl}Neden: " + reason
        ...
        text += symbol + " -> " + f"{{price:.2f}}" + " TL{nl}"
        text += "Lot Sayisi: " + str(shares) + " Lot{nl}"
        text += "Cikis Tutari: " + f"{{total_val:.2f}}" + " TL{nl}{nl}"
        text += "K/Z (Tutar): " + f"{{pnl_val:+.2f}}" + " TL{nl}"
        text += "K/Z (%): %" + f"{{pnl_pct:+.2f}}" + "{nl}"
        if reason:
            text += "{nl}Neden: " + reason
        return send_telegram_message(text)
"""
