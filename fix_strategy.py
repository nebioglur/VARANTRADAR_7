...
CODE_TO_REPLACE = '''    def check_custom_strict_strategy(self, df, direction: str = "AL") -> bool:
        ...
        try:
            ...
            prev_momentum = float(momentum.iloc[-2]) if len(momentum) > 1 else current_momentum
            ...
            prev_adx = float(adx.iloc[-2]) if len(adx) > 1 else current_adx
            
            # Evaluate Conditions (SIMULTANEOUS CROSSOVERS)
            if direction == "AL":
                if not (prev_adx <= 25 and current_adx > 25): return False
                if not (prev_momentum <= 0 and current_momentum > 0): return False
...
# The file might be corrupted at the end now. I will just truncate after start_idx and write the new code.
