...
def background_scanner():
    ...
    def process_notifications(results):
        ...
        for tavan in results.get("tavan_adaylari", []):
            ...
            if sent_tavan.get(sym) != today_str:
                notif.send_tavan_alert(sym, tavan.get("Score", 0), tavan.get("Report", ""), tavan.get("Position"))
    ...
    if not GLOBAL_DASHBOARD_CACHE:
        try:
            ...
            fast_results["opportunities_1h"] = fast_1h_res.get("opportunities_1h", [])
            fast_results["tavan_adaylari"] = fast_1h_res.get("tavan_adaylari", [])
            ...
            fast_results["signals_5m"] = fast_5m
            
            GLOBAL_DASHBOARD_CACHE = fast_results
            save_dashboard_cache(fast_results)
            
            # Telegram bildirimlerini gonder
            process_notifications(fast_results)
    ...
    while True:
        try:
            ...
            results["opportunities_1h"] = res_1h.get("opportunities_1h", [])
            results["tavan_adaylari"] = res_1h.get("tavan_adaylari", [])
            
            print("[BACKGROUND] 1h taraması tamamlandı. BIST50 için 5m taraması başlıyor...")
            valid_bist50_loop = [s for s in BIST50_SYMBOLS if s in daily_stats]
            res_5m = scanner.scan_pool_bulk_5m(valid_bist50_loop)
            results["signals_5m"] = res_5m
            
            GLOBAL_DASHBOARD_CACHE = results
            save_dashboard_cache(results)
            
            # Telegram bildirimlerini gonder
            process_notifications(results)
