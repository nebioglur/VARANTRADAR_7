...
function renderStatsMode() {
    ...
    if (current_stats_mode === 'cumulative') {
        ...
        if (el('stats-tab-total-candidates-sub')) el('stats-tab-total-candidates-sub').innerText = \Toplam \ Öneri\;
        if (el('stats-tab-tavan-rate')) el('stats-tab-tavan-rate').innerText = \%\;
        if (el('stats-tab-tavan-cnt-sub')) el('stats-tab-tavan-cnt-sub').innerText = \ Tavan\;
        if (el('stats-tab-plus5-rate')) el('stats-tab-plus5-rate').innerText = \%\;
        if (el('stats-tab-plus5-cnt-sub')) el('stats-tab-plus5-cnt-sub').innerText = \ Hisse\;
        if (el('stats-tab-avg-max-gain')) el('stats-tab-avg-max-gain').innerText = \+ %\;
        if (el('stats-tab-avg-close-sub')) el('stats-tab-avg-close-sub').innerText = \Kapanış: %\;
        if (el('stats-tab-warrant-avg-gain')) el('stats-tab-warrant-avg-gain').innerText = \+ %\;
        if (el('stats-tab-pos-count')) el('stats-tab-pos-count').innerText = \ Adet\;
        if (el('stats-tab-pos-avg')) el('stats-tab-pos-avg').innerText = \(Ort. +%\)\;
        if (el('stats-tab-neg-count')) el('stats-tab-neg-count').innerText = \ Adet\;
        if (el('stats-tab-neg-avg')) el('stats-tab-neg-avg').innerText = \(Ort. %\)\;
        let netPct = summ.net_profit_pct || summ.cumulative_avg_closing_gain_pct || 0;
        let netSign = netPct > 0 ? '+' : '';
        if (el('stats-tab-net-pct')) el('stats-tab-net-pct').innerText = \%\;
        ...
        if (el('stats-tab-elite-pos-count')) el('stats-tab-elite-pos-count').innerText = \ Adet\;
        if (el('stats-tab-elite-pos-avg')) el('stats-tab-elite-pos-avg').innerText = \(Ort. +%\)\;
        if (el('stats-tab-elite-neg-count')) el('stats-tab-elite-neg-count').innerText = \ Adet\;
        if (el('stats-tab-elite-neg-avg')) el('stats-tab-elite-neg-avg').innerText = \(Ort. %\)\;
        let eliteNet = summ.elite_net_profit_pct || 0;
        let eliteSign = eliteNet > 0 ? '+' : '';
        if (el('stats-tab-elite-net-pct')) el('stats-tab-elite-net-pct').innerText = \%\;
