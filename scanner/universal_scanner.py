...
class UniversalScanner:
    ...
    def scan_pool_bulk_1h(self, symbols: List[str], daily_stats: Dict[str, Any] = None) -> Dict[str, List[Dict[str, Any]]]:
        ...
        print(f"[SCANNER 1H] Tarama tamamlandı. {len(opportunities)} fırsat, {len(tavan_adaylari)} tavan adayı bulundu.")
        return {"opportunities_1h": opportunities, "tavan_adaylari": tavan_adaylari}

    def scan_pool_bulk_5m(self, symbols: List[str]) -> List[Dict[str, Any]]:
        """
        5 Dakikalık periyotta RSI 50 (Yukarı) ve 70 (Aşağı) kesişimlerini yakalar.
        """
        if not symbols:
            return []

        print(f"[SCANNER 5M] {len(symbols)} hisse 5 dakikalık periyotta indiriliyor (Kısa Vade RSI Trade)...")
        # 5m data can only be downloaded for max 60 days. 5 days is enough for RSI(14).
        ...
        for sym in symbols_to_process:
            ...
            # Analizi çağır
            res_5m = self.tech_engine.analyze_5m_rsi_strategy(sym, df)
            if res_5m:
                # Sinyal Yönüne göre (AL veya SAT) Custom Strict Stratejiyi uygula
                signal_direction = res_5m.get("Signal", "AL")
                is_match, bars_ago, timestamp_str = self.tech_engine.check_custom_strict_strategy(df, direction=signal_direction)
                if is_match:
                    res_5m["Time_Label"] = "ŞİMDİ (Sıcak)" if bars_ago == 0 else f"{bars_ago} Bar Önce"
                    res_5m["Bars_Ago"] = bars_ago
                    signals.append(res_5m)
                    
        # AL sinyallerini öne, sonra SAT sinyallerini getir, kendi içlerinde saate göre sırala (en yeni)
        signals = sorted(
            signals,
            key=lambda x: (0 if x.get("Signal") == "AL" else 1, x.get("Time", "")),
            reverse=True
        )
        
        print(f"[SCANNER 5M] Tarama tamamlandı. {len(signals)} sinyal (RSI Kesişimi) bulundu.")
        return signals
