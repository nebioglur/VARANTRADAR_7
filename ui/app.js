...
function switchMainTab(tabName, btnElement) {
    ...
    if (tabName === 'stats') {
        fetchStatsTabData();
        // Ä°statistik sekmesi aÃ§Ä±kken 60 saniyede bir otomatik gÃ¼ncelle
    ...
    if (tabName === 'varant') {
        // Varant sekmesini yÃ¼kle (mevcut analiz varsa)
    ...
    if (tabName === 'simulation') {
        fetchSimulationData();
        // SimÃ¼lasyon sekmesi aÃ§Ä±kken 60 saniyede bir otomatik gÃ¼ncelle
...
async function startRadar(type) {
    ...
    try {
        ...
        if (!data.results || data.results.length === 0) {
            if (tbodyEl) tbodyEl.innerHTML = `<tr><td colspan='4' style='text-align:center; color: var(--text-muted); padding:2rem;'><i class="fa-solid fa-circle-exclamation text-yellow" style="font-size:1.5rem; display:block; margin-bottom:8px;"></i>Bu kategoride henÃ¼z gÃ¼Ã§lÃ¼ sinyal oluÅŸmadÄ±. Sistem piyasayÄ± izlemeye devam ediyor.</td></tr>`;
            return;
        }
            ...
            if (data.dashboard_data && data.dashboard_data.tavan_adaylari) {
                ...
                data.dashboard_data.tavan_adaylari.forEach(item => {
                    if (!window.previousTavanSymbols.has(item.Symbol) && item.Score >= 90) {
                        ...
                        if(newSymbolsCount < 3) showToast("?? " + item.Symbol + " Tavan Adaylarına Girdi (" + item.Score + " Puan)");
                        newSymbolsCount++;
                    }
                    window.previousTavanSymbols.add(item.Symbol);
                });
    ...
    if (bPct < 35) { breadthEl.style.color = "#ef4444"; breadthEl.innerText += " (Zayıf)"; }
                ...
                if (aiEl) {
                    ...
                    if (data.xu100_change <= -1.5) {
                        ...
                        aiComment = "ğŸš€ Endeks Ã§ok gÃ¼Ã§lÃ¼. Tavan adaylarÄ±nda hedefler daha esnek tutulabilir. BoÄŸa piyasasÄ± aktif.";
    ...
    for (let cat in cats) {
        ...
        displayItems.forEach(res => {
            ...
            if (cat === 'arge_tavan') {
                ...
                if (res.ORB_Breakout) {
                    evreStr += `<div style="margin-top:2px;"><span title="AÃ§Ä±lÄ±ÅŸ AralÄ±ÄŸÄ± KÄ±rÄ±lÄ±mÄ± (ORB): Hisse gÃ¼nÃ¼n ilk saatlerindeki tepe noktasÄ±nÄ± hacimli ÅŸekilde yukarÄ± kÄ±rdÄ±." style="background:rgba(56, 189, 248, 0.15); color:#38bdf8; border:1px solid #38bdf8; padding:1px 4px; border-radius:3px; font-size:0.66rem; font-weight:700; cursor:help;"><i class="fa-solid fa-bullseye"></i> ORB AÃ§Ä±lÄ±ÅŸ KÄ±rÄ±lÄ±mÄ±</span></div>`;
                }

                if (res.V_Reversal) {
                    evreStr += `<div style="margin-top:3px;"><span title="V-DÃ¶nÃ¼ÅŸ: GÃ¼n iÃ§i dip seviyesinden Ã§ok hÄ±zlÄ± ve gÃ¼Ã§lÃ¼ bir ÅŸekilde toparlanÄ±p ivme kazandÄ±." style="background:rgba(168, 85, 247, 0.2); color:#c084fc; padding:2px 5px; border-radius:3px; font-size:0.68rem; font-weight:700; cursor:help;"><i class="fa-solid fa-bolt-lightning"></i> V-DÃ¶nÃ¼ÅŸ +%${(res.V_Power||0).toFixed(1)}</span></div>`;
                }
                ...
                if (res.Breakdown_Risk) {
                    hacimStr += `<br><span style="background:rgba(239, 68, 68, 0.25); color:var(--accent-red); padding:1px 5px; border-radius:3px; font-size:0.68rem; font-weight:800;"><i class="fa-solid fa-triangle-exclamation"></i> Ã‡Ã–ZÃœLME RÄ°SKÄ°</span>`;
                    ...
                    hacimStr += `<br><span style="background:rgba(16, 185, 129, 0.2); color:var(--accent-green); padding:1px 5px; border-radius:3px; font-size:0.68rem; font-weight:700;"><i class="fa-solid fa-dumbbell"></i> BoÄŸa GÃ¼cÃ¼</span>`;
                }
                
                if (res.Domino_Sector && res.Domino_Peers && res.Domino_Peers.length > 0) {
                    let pStr = res.Domino_Peers.slice(0, 2).map(p => '#' + p).join(' ');
                    hacimStr += `<div style="font-size:0.68rem; color:#94a3b8; margin-top:2px;" title="SektÃ¶rel Domino KardeÅŸ Hisseleri"><i class="fa-solid fa-chess-knight"></i> ${res.Domino_Sector}: ${pStr}</div>`;
                }
                
                // AI Skor, Teyit Skoru, Ã‡ift Tavan ve Varant EÅŸleÅŸmesi
                ...
                if (res.FOMO_Level) {
                    scoreStr += `<div style="font-size:0.7rem; color:${res.FOMO_Color}; font-weight:700; margin-top:4px;" title="FOMO Ä°ndikatÃ¶rÃ¼: ${res.FOMO_Score.toFixed(1)}"><i class="fa-solid fa-fire-flame-curved"></i> FOMO: ${res.FOMO_Level}</div>`;
                }
                ...
                if (res.Streak_Score) {
                    scoreStr += `<div style="font-size:0.68rem; color:#38bdf8;" title="Ã‡ift Tavan Ä°htimali"><i class="fa-solid fa-link"></i> %${res.Streak_Score} Seri</div>`;
                }
                
                // YENÄ°: Ä°ndikatÃ¶r GÃ¼Ã§lendirmesi (RSI & MACD)
                ...
                if (res.Position) {
                    scoreStr += `
                        <div style="font-size:0.7rem; display:flex; gap:4px; flex-wrap:wrap; margin-top:2px;">
                            <span style="background:rgba(239, 68, 68, 0.15); color:var(--accent-red); padding:1px 4px; border-radius:3px; font-weight:600;" title="Ä°z SÃ¼ren Stop"><i class="fa-solid fa-shield-halved"></i> â‚º${res.Position.SL}</span>
                        </div>
                    `;
                }
        ...
        if (opp1hItems.length === 0) {
            ...
            opp1hItems.forEach(res => {
                ...
                tr.innerHTML = `
                    ...
                    <td><a href="/?symbol=${res.Symbol}&tab=graphic" target="_blank" class="btn btn-sm btn-outline-primary" style="text-decoration:none;" onclick="event.preventDefault(); openGraphicTab('${res.Symbol}'); switchMainTab('home', document.querySelector('.nav-btn.active'));">Grafikte AÃ§</a></td>
                `;
...
window.addEventListener('DOMContentLoaded', () => {
    ...
    // Ana sayfa istatistiklerini 60 saniyede bir otomatik gÃ¼ncelle
...
async function fetchStatsTabData() {
    ...
    try {
        ...
        if (data.status === 'success' || data.summary) {
            ...
            // Yeni Eklenen KapanÄ±ÅŸ KÃ¢r/Zarar DÃ¶kÃ¼mÃ¼ KutularÄ±
            ...
            // Elite (Score 100) KapanÄ±ÅŸ KÃ¢r/Zarar DÃ¶kÃ¼mÃ¼ KutularÄ±
            ...
            // PopÃ¼le Et: Daily History Tablosu
            if (dailyTbody) {
                dailyTbody.innerHTML = '';
                if (history.length === 0) {
            ...
            if (tbody) {
                ...
                if (trades.length === 0) {
                    ...
                    trades.forEach(t => {
                        ...
                        const statusStr = isClosed ? (t.exit_reason || 'KapandÄ±') : '<span style="color:var(--accent-yellow); font-weight:bold;"><i class="fa-solid fa-spinner fa-spin"></i> AÃ‡IK POZÄ°SYON</span>';
