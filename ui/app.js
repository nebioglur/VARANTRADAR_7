            ...
            if (data.dashboard_data && data.dashboard_data.tavan_adaylari) {
                ...
                data.dashboard_data.tavan_adaylari.forEach(item => {
                    if (!window.previousTavanSymbols.has(item.Symbol) && item.Score >= 90) {
                        ...
                        if(newSymbolsCount < 3) showToast("?? " + item.Symbol + " Tavan Adaylar�na Girdi (" + item.Score + " Puan)");
                        newSymbolsCount++;
                    }
                    window.previousTavanSymbols.add(item.Symbol);
                });
...
// Toast Pop-up
function __disabled_showToast(message, icon = "fa-bell") {
...
// X-Ray Modal
function __disabled_openXRayModal(symbol) {
    ...
    if (stockData) {
        ...
        const reason = stockData.Report || stockData.Reason || 'Sistem taraf�ndan yakaland�.';
        ...
        body.innerHTML = 
            ...
            <button class="btn-primary" style="width:100%; margin-top:15px;" onclick="document.getElementById('symbol-input').value='+symbol+'; analyzeSymbol(); closeXRayModal(event);">Detayl� Grafi�e Git</button>
        ;
    } else {
        body.innerHTML = <div style="text-align:center; color:var(--text-muted);">Veri bulunamad�.</div>;
    }
...
function openXRayModal(symbol) {
    ...
    if (stockData) {
        ...
        let priceStr = typeof price==='number' ? price.toFixed(2) : price;
        let rsiStr = typeof rsi==='number' ? rsi.toFixed(1) : rsi;
        let volStr = typeof vol==='number' ? vol.toFixed(1) : vol;
        ...
        body.innerHTML = `
            <div class="xray-grid">
                ...
                <div class="xray-box">
                    <h4>RSI (14)</h4>
                    <div class="val" style="color:${rsiColor}">${rsiStr}</div>
                </div>
                <div class="xray-box">
                    <h4>Hacim İvmesi</h4>
                    <div class="val" style="color:var(--accent-yellow)">${volStr}x</div>
                </div>
                <div class="xray-box full-width" style="text-align:left; font-size:0.9rem;">
                    <h4>🧠 Komite Raporu</h4>
                    <div style="color:var(--text-muted); margin-top:5px; line-height:1.4;">${reason}</div>
