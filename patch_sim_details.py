...
# Replace the notify_sim_trade definition and body
pattern_tg = r'def notify_sim_trade\(symbol.*?return send_telegram_message\(text\)'
replacement_tg = r'''def notify_sim_trade(symbol: str, action: str, price: float, pnl_pct: float = 0.0, reason: str = "", date_str: str = "", trade: dict = None) -> bool:
    import json, os
    ...
    if "AL" in action:
        text = (f"?? <b>SÝMÜLASYON {action}</b>\\n\\n"
                ...
                f"?? <b>Lot Sayýsý:</b> {shares} Lot\\n"
                f"?? <b>Toplam Tutar:</b> {total_val:.2f} TL\\n\\n"
                f"?? <b>Kâr Al (TP):</b> {tp1:.2f} TL (+%{tp_pct:.1f})\\n"
        ...
        text = (f"?? <b>SÝMÜLASYON {action}</b>\\n\\n"
                f"?? <b>{symbol}</b> ? {price:.2f} TL\\n"
                f"?? <b>Lot Sayýsý:</b> {shares} Lot\\n"
                f"?? <b>Çýkýþ Tutarý:</b> {total_val:.2f} TL\\n\\n"
...
# Replace AL notifications to pass `active_trades[-1]`
...
# Replace SAT notifications to pass `trade`
s_text = s_text.replace(
