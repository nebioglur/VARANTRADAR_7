...
target = """            const maxG = item.max_gain_pct !== undefined ? item.max_gain_pct.toFixed(2) : (item.max_gain !== undefined ? item.max_gain : '-');
...
replacement = """            const maxG = item.max_gain_pct !== undefined ? item.max_gain_pct.toFixed(2) : (item.max_gain !== undefined ? item.max_gain : '-');
            ...
            card.innerHTML = `
                ...
                <div style="display:flex; justify-content:space-between; font-size:0.75rem; margin-bottom:8px; background:rgba(0,0,0,0.2); padding:4px; border-radius:4px;">
                    <span style="color:var(--text-muted);">Sinyal Getirisi:</span>
...
if target in c:
    ...
    with open('ui/app.js', 'w', encoding='utf-8') as f:
        f.write(c)
    print("Replaced Zirve Kâr successfully.")
else:
    print("Target not found. Let's try searching via find().")
    idx = c.find('Zirve Kâr')
    if idx != -1:
        print("Found Zirve Kâr at index", idx)
