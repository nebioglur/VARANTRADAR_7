...
class WinRateEngine:
    ...
    def get_performance_stats(cls) -> Dict[str, Any]:
        """Tum sistemin genel basari metriklerini gercek verisinden dondurur."""
        try:
            # BUG FIX: Sadece mevcut haftayi degil, tum tarihsel veriyi kullan
            ...
            for day in daily_breakdown[:5]:  # Son 5 gun
                ...
                # Items'i dogrudan audit arsivinden cek
                day_audit = all_audits.get(day_date, {})
                candidates = day_audit.get("items", [])
                ...
                for c in candidates[:3]:  # Her gunden max 3 sinyal
                    ...
                    recent_signals.append({
                        ...
                        "entry_price": c.get("morning_price", ""),
                        "target_price": "",
                        "exit_price": c.get("closing_price", c.get("daily_high", "")),
                        "pnl_pct": pnl,
                        "status": status,
                        "duration": c.get("snapshot_time", ""),
            ...
            return {
                ...
                "daily_breakdown": proper_daily_breakdown,
